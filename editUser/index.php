...
// echo "$user_id";
if ($logged_role_id != 3 && $logged_role_id != 4 && $Loggedin_user_id != $user_id) {
    header('location:../dashboard/?sort=month');
}
$sql = "SELECT user_first_name,user_last_name,user_email,user_country_name,
user_state_name,user_city_name,user_street_address,user_phone,user_password,
user_role_id,user_gender FROM em_users WHERE user_id=$user_id";
$result = mysqli_query($con, $sql);
...
$get_role = "SELECT role_name,role_id FROM em_roles WHERE role_id=$user_role_id";
...
if (isset($_POST['Submit'])) {
    ...
    if (empty($_POST['email'])) {
        ...
        if (!filter_var($new_email, FILTER_VALIDATE_EMAIL)) {
            ...
            if ($user_email != $new_email) {
                $sql_check = "SELECT user_first_name,user_last_name,user_email,user_country_name,
                user_state_name,user_city_name,user_street_address,user_phone,user_password,
                user_role_id,user_gender from `em_users` WHERE user_email='$new_email'";
    ...
    if ($_POST['country'] === 'select') {
        ...
        $user_country_id = $_POST['country'];
        $sql = "SELECT country_name,country_id FROM `em_countries` WHERE country_id='$user_country_id'";
        $result = mysqli_query($con, $sql);
        $row = mysqli_fetch_array($result);
    ...
    if ($_POST['city'] === 'select') {
        ...
        $sql = "SELECT city_name,city_id FROM `em_cities` WHERE city_id='$user_city_id'";
        $result = mysqli_query($con, $sql);
        $row = mysqli_fetch_array($result);
    ...
    if (empty($errors)) {
        $sql = "SELECT role_name,role_id,role_slug FROM em_roles WHERE role_name='$role_name'";
        $result = mysqli_query($con, $sql);
        while ($row = mysqli_fetch_assoc($result)) {
            $role_id = $row['role_id'];
            $role_name = $row['role_name'];
            $role_slug = $row['role_slug'];
            $sql1 = "UPDATE `em_users` SET user_role_id='$role_id',
            user_first_name='$user_firstname',user_city_name='$user_city_id',user_state_name='$user_state_id',user_last_name='$user_lastname',
            user_email='$new_email',user_phone='$user_phone',user_password='$changed_password',
            user_street_address='$user_street_address',user_country_name='$user_country_id',
            user_gender='$gender' WHERE user_id=$user_id";
            $res = mysqli_query($con, $sql1);
            if ($res) {
                // echo "success";
                if ($logged_role_id != 3 && $logged_role_id != 4) {
                    $_SESSION['email'] = $new_email;
                    $_SESSION['password']=$changed_password;
                    header('location:../showProfile');
                } else {
                    if ($email != $new_email && $Loggedin_user_id == $user_id) {
                        $_SESSION['email'] = $new_email;
                        header('location:../showProfile');
                    } else {
                        $_SESSION['email'] = $email;
                        header('location:../list-users/?sort=user_updatedAt');
                    }
                }
                            ...
                            <div class="input-field">
                                <div class="select">
                                    <select id="role" name="role_name">
                                        ...
                                        $sql = "SELECT role_id,role_name,role_slug from em_roles";
                                        $result = mysqli_query($con, $sql);
                                        if ($result) {
                                            while ($row = mysqli_fetch_assoc($result)) {
                                                $role_name = $row['role_name'];
                            ...
                            <div class="input-field">
                                ...
                                <div class="select">
                                    <select name="state" id="ste" onclick="fetch_city()">
                                        ...
                                        <?php
                                        $sql = "SELECT state_id,state_name FROM `em_states` WHERE country_id='$c_id'";
                                        $result = mysqli_query($con, $sql);
                                        while ($row = mysqli_fetch_assoc($result)) {
                            ...
                            <div class="input-field">
                                <div class="select">
                                    <select name="city" id="city">
                                        ...
                                        <?php
                                        $sql = "SELECT city_name,city_id FROM `em_cities` where state_id='$s_id'";
                                        $result = mysqli_query($con, $sql);
                                        while ($row = mysqli_fetch_assoc($result)) {
