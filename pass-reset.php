...
$find_email = "SELECT user_email FROM `employee_users` WHERE user_id='$user_id'";
...
if (isset($_POST['Reset'])) {
  ...
  $sql = "SELECT user_firstname,token,user_email,user_id FROM `employee_users` WHERE user_email='$user_email'";
          ...
          if (!empty($updated_password)) {
            if (preg_match($pattern, $updated_password)) {
              if ($updated_password == $confirm_pass) {
                ...
                $sql = "SELECT * FROM `employee_users` WHERE user_email='$user_email'";
                $result = mysqli_query($con, $sql);
                if (mysqli_num_rows($result) > 0) {
                  $update = "UPDATE `employee_users` SET user_password='$updated_password' WHERE user_email='$email'";
                  ...
                  if ($update_execute) {
                    ...
                    $delete_token = "UPDATE `employee_users` SET token=null WHERE user_email='$user_email'";
          ...
          <form method="POST" class="margin_bottom" role="form" autocomplete="on">
            <div class="form-group">
              ...
              <input type="password" id="new_password" name="new_password" placeholder="Enter your password"
                class="form-control" value=<?php echo $_POST['new_password']; ?>>
              <div id="message">
