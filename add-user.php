...
if (isset($_POST['Submit'])) {
    ...
    if ($_POST['country'] === 'select') {
        ...
        $user_country_id = $_POST['country'];
        $sql = "SELECT country_id,country_name FROM `countries` WHERE country_id='$user_country_id'";
        $result = mysqli_query($con, $sql);
    ...
    if ($_POST['city'] === 'select') {
        ...
        $sql = "SELECT city_id,city_name FROM `cities` WHERE city_id='$user_city_id'";
        $result = mysqli_query($con, $sql);
        $row = mysqli_fetch_array($result);
    ...
    if (empty($errors)) {
        $sql = "SELECT role_id,role_name FROM `employee_roles` WHERE role_name='$role_name'";
        $result = mysqli_query($con, $sql);
        while ($row = mysqli_fetch_assoc($result)) {
            $role_id = $row['role_id'];
            $role_name = $row['role_name'];
            $role_slug = $row['role_slug'];
            $sql1 = "INSERT INTO `employee_users` (user_role_id,gender,user_state_id,user_city_id,user_firstname,user_lastname,user_email,user_phone,user_password,user_street_address,user_country_id,Active_status)
        VALUES ('$role_id','$gender','$user_state_id','$user_city_id','$user_firstname','$user_lastname','$user_email','$user_phone','$user_password','$user_street_address','$user_country_id','Active')";
            ...
            if ($res) {
                echo "success";
                header('location:list-users.php?sort=createdAt&search_val=&page=1');
            } else {
                echo "error";
            }
        }
...
<body>
    ...
    <div class="content">
        <div class="wrapper">
            <div class="bedcram">
                <ul>
                    <li><a href="dashboard.php?sort=months">Home</a></li>
            ...
            <div class="right_side_content">
                ...
                <div class="list-contet">
                    <div class="error-message-div error-msg" id="error_msg" style="display: none;"><img
                            src="images/unsucess-msg.png"><strong>UnSucess!</strong> Please enter correct Details</div>
