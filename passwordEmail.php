...
function sendPasswordEmail($name, $email, $token, $id, $usage, $header)
{
  ...
  if ($result) {
    ...
    try {
      ...
      //       href = 'http://localhost/crud_design/pass-reset.php?token={{token}}&user_id={{user_id}}'>Reset Password</a></button>
      //       <p><a style='text-decoration:none;' href = 'http://localhost/crud_design/signin.php'>That wasn't me!</a></p>
      //       <p>Facing difficuly in button? Copy the link: <a style='color-blue;'>http://localhost/crud_design/pass-reset.php?token={{token}}&user_id={{user_id}}</a></p>
